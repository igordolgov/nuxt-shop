...
.modal(v-if="isOpen" class="modal-open")
  .modal-box.max-w-2xl.relative
    ...
    h3.text-xl.font-bold.mb-2 Добавить новый товар
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      compressImage(e.target.result, 0.7).then(compressedImage => {
        form.value.image = compressedImage
        formError.value = ''
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log('📤 Отправка данных...', { newProduct })
    ...
    // Принудительно обновляем список товаров для синхронизации
    await loadProducts(true)
    
    // Показываем успешное уведомление
    notify.success('Товар успешно добавлен')
    
    // Эмитим событие с новым товаром для немедленного обновления UI
    ...
    formError.value = error.message || 'Произошла ошибка при добавлении товара'
    uploadProgress.value = 0
    isSubmitting.value = false
    
    // Показываем ошибку через систему уведомлений
    notify.error(error.message || 'Произошла ошибка при добавлении товара')
  }
}
